...
def parse_call_id_list(
    call_ids: Optional[str] = Query(None, description="comma-separated list of int call ids")
) -> List[str]:
    """parse a string of (comma-separated) call ids"""
    if not call_ids:
        return None
    parsed_call_ids = [int(cid.strip()) for cid in re.split(r"[^0-9]+", call_ids)]
...
def parse_field_list(
    fields: Optional[str] = Query(None, description="comma-separated list of field ('key') names. if empty, search all")
) -> List[str]:
...
def parse_id_list(
    ids: Optional[str] = Query(None, description="comma-separated list of id names. if empty, search all")
) -> List[str]:
    """parse a string of (comma-separated) field names"""
    if not ids:
        return None
...
def parse_title_list(
    titles: Optional[str] = Query(None, description="comma-separated list of titles. if empty, search all")
) -> List[str]:
    """parse a string of (comma-separated) field names"""
    if not titles:
        return None
...
def parse_origins_list(origins: Optional[str]) -> List[str]:
    """parse a string of (comma-separated) field names"""
    if not origins:
        return [""]
...
def parse_report(report: str, sections: list) -> dict:
    ...
    for i in range(len(sections)):
        ...
        try:
            if i < len(sections) - 1:
                match = re.search(f"\[{section}\](.*)\[{sections[i+1]}\]", report, re.DOTALL | re.IGNORECASE)
            else:
                match = re.search(f"\[{section}\](.*)", report, re.DOTALL | re.IGNORECASE)

            if match:
                lines = [
                    re.sub("^[0-9\-\.]+\s+", "", line).strip()
